...
export class AppointmentServiceAngularComponentComponent implements OnInit {
 ...
 Service_Name2: string = '';
  ...
  constructor(public http: HttpClient, public data: DataService, private route: ActivatedRoute) {
    this.files = [];
    this.http.get('https://localhost:7288/AppointmentService/GetAllService')
      .subscribe(data => {
        this.items = data;
 ...
 Appointmentchange() {
    ...
    this.Date2 = "";
    this.http.get('https://localhost:7288/AppointmentService/GetAppointment/' + this.Appointment_ID2)
      .subscribe(data => {
        if (data != "") {
  ...
  showItems() {
    this.http.get('https://localhost:7288/AppointmentService/GetIService/' + this.Appointment_ID2)
      .subscribe(data => {
        this.items = data;
        console.log(this.items);
      });
  ...
  onFileChanged(event: any) {
    ...
    this.http.post('https://localhost:7288/AppointmentService/Post/', formData).subscribe(data => {
  ...
  deleteAll(): void {
    ...
    var data = {};
    this.http.post<any>('https://localhost:7288/AppointmentService/RemoveAppointmentServiceVm/' + this.Appointment_ID2, JSON.stringify(data), httpOptions).subscribe(data => {
  ...
  saveAll(): void {
    ...
    this.http.post<any>('https://localhost:7288/AppointmentService/AddAppointmentServiceVm', JSON.stringify(data), httpOptions).subscribe(data => {
      window.location.href = 'https://localhost:4200/';
    });
  }
  myalert(data: string) {
